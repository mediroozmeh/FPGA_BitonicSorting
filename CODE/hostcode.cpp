...
int main(int argc, char** argv)
{
	...
	cl_kernel sortLocalKernel, sortLocal1Kernel, mergeGlobalKernel, mergeLocalKernel;
	...
	sortLocalKernel = clCreateKernel(program, "bitonicSortLocal", &err); 
	check_err(err, "bitonicSortLocal kernel"); 
	...
	//unsigned int arrayLength = LOCAL_SIZE_LIMIT;
...
//	unsigned int arrayLength= LOCAL_SIZE_LIMIT;
	...
	if (factorRadix2(log2L, arrayLength) != 1 || factorRadix2(log2L, LOCAL_SIZE_LIMIT) != 1) {
	    printf("only power of 2 sizes supported");
	    exit(-1);
	}
	if (arrayLength <= LOCAL_SIZE_LIMIT) {
	    err |= clSetKernelArg(sortLocalKernel, 0, sizeof(cl_mem), &dst_key); 
	    err |= clSetKernelArg(sortLocalKernel, 1, sizeof(cl_mem), &dst_val); 
	    err |= clSetKernelArg(sortLocalKernel, 2, sizeof(cl_mem), &src_key); 
	    err |= clSetKernelArg(sortLocalKernel, 3, sizeof(cl_mem), &src_val); 
	    err |= clSetKernelArg(sortLocalKernel, 4, sizeof(cl_uint), &arrayLength); 
	    err |= clSetKernelArg(sortLocalKernel, 5, sizeof(cl_uint), &dir); 
	    check_err(err, "arg"); 
	 ...
	 // global= LOCAL_SIZE_LIMIT;
          //local = LOCAL_SIZE_LIMIT;
	  
	    printf("STARTING KERNEL SortLocal\n"); 
	    err |= clEnqueueNDRangeKernel(command_queue, sortLocalKernel, 1, NULL, (size_t *)&global,(size_t *) &local, 0, NULL, NULL); 
	    check_err(err, "exec"); 
...
//{
 // printf(" FOR I = %d : dst_key: %d, dst_val: %d \n", i , dstkey[i] , dstval[i]);
...
//} 
	...
	clReleaseKernel(sortLocalKernel);
